...
FIELDS: tuple[ConfigFieldSpec, ...] = (
    ...
    ConfigFieldSpec(
        ...
        secret=True,
        description="Bearer token or API key for voyage.prod.telepub.cn OpenAI-compatible chat.",
    ),
    ConfigFieldSpec(
        ...
        description="Root URL for OpenAI client (requests go to …/chat/completions).",
    ),
    ConfigFieldSpec(
        "TELEPUB_VOYAGE_MODELS",
        "Telepub Voyage Static Model IDs",
        "providers",
        settings_attr="telepub_voyage_models",
        description=(
            "Comma-separated upstream model ids (multiple allowed). Each id must cover "
            "every telepub_voyage/… route used in MODEL, MODEL_OPUS, MODEL_SONNET, or MODEL_HAIKU. "
            "Values are merged with GET /models when that endpoint works."
        ),
    ),
    ConfigFieldSpec(
        "LM_STUDIO_BASE_URL",
