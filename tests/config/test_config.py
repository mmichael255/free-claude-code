...
class TestSettings:
    ...
    def test_telepub_voyage_base_url_rejects_chat_completions_suffix(self, monkeypatch):
        """TELEPUB_VOYAGE_BASE_URL must not include /chat/completions (OpenAI SDK appends it)."""
        from config.settings import Settings

        monkeypatch.setenv(
            "TELEPUB_VOYAGE_BASE_URL",
            "https://voyage.prod.telepub.cn/voyage/api/chat/completions",
        )
        with pytest.raises(ValidationError, match="do not append"):
            Settings()
...
class TestPerModelMapping:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                {"MODEL": "telepub_voyage/my-upstream-model"},
                "telepub_voyage/my-upstream-model",
                None,
            ),
        ],
    )
    ...
    def test_parse_provider_type(self):
        ...
        assert Settings.parse_provider_type("telepub_voyage/gpt-4") == "telepub_voyage"
    ...
    def test_parse_model_name(self):
        ...
        assert Settings.parse_model_name("telepub_voyage/gpt-4") == "gpt-4"
